...
class GetBasis(nn.Module):
    def __init__(self, sizeP, tranNum=4, inP=None):
        ...
        self.register_buffer("Mask", Mask.reshape(sizeP,sizeP,1,1)) 
        ...
        self.k = torch.arange(-(inP//2),inP//2+1, device=device).reshape(1,1,1,inP,1)
        # print('k', self.k.device) # cpu
        self.l = torch.arange(-(inP//2),inP//2+1, device=device).reshape(1,1,1,1,inP)
    ...
    def forward(self):
        ...
        Basis = Basis.reshape(Basis.size(0),Basis.size(1),Basis.size(2),Basis.size(3)*Basis.size(4))*self.Mask
