...
def GetBasis_PCA(sizeP, tranNum=8, inP=None, Smooth = True):
    ...
    Mask = Mask.reshape([sizeP,sizeP,1,1,1])
    ...
    # k = np.reshape(np.arange(-inp-1, inp+2),[1,1,1,inP+2,1])
    # l = np.reshape(np.arange(-inp-1, inp+2),[1,1,1,1,inP+2])
    
    # print(X[:,:,0,0,0])
    Basis = (BicubicIni(X-k)*BicubicIni(Y-l))*Mask
    # print(Basis[:,:,1,2,2])
    # print(Basis[:,:,0,2,2])
    Basis = Basis.reshape([sizeP*sizeP*tranNum, inP*inP])
    U,S,VT = np.linalg.svd(np.matmul(Basis.T,Basis))

    Rank   = np.sum(S>0.0001)
    Basis = np.matmul(np.matmul(Basis,U[:,:Rank]),np.diag(1/np.sqrt(S[:Rank]+0.0000000001))) 
    Basis = np.reshape(Basis,[sizeP, sizeP, tranNum, Rank])
    
    temp = np.reshape(Basis, [sizeP*sizeP, tranNum, Rank])
...
#        iniw = torch.randn(outNum, inNum, self.expand, self.Basis.size(3))*0.03
        iniw = Getini_reg(inP, inNum, outNum, self.expand)
        self.weights = nn.Parameter(iniw, requires_grad=True)
        if padding == None:
            self.padding = 0
        else:
