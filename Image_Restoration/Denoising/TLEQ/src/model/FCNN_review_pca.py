...
#    X = X.unsqueeze(3).unsqueeze(4)
    ...
    v = np.pi/inP*(inP-1)
#    p = inP/2
    p1 = int(np.ceil((inP+1)/2))
    p  = p1-1

    k = np.reshape(np.arange(p)+1.0,[1,1,1,p,1])
    l = np.reshape(np.arange(p)+1.0,[1,1,1,1,p])
    k1 = np.reshape(np.arange(p1),[1,1,1,p1,1])
    l1 = np.reshape(np.arange(p1),[1,1,1,1,p1])
    
    Basis1 = np.cos(k1*v*X)*np.cos(l1*v*Y)
    Basis2 = np.cos(k1*v*X)*np.sin(l*v*Y)
    Basis3 = np.sin(k*v*X)*np.cos(l1*v*Y)
    Basis4 = np.sin(k*v*X)*np.sin(l*v*Y)
#    print(np.shape(Basis1))
        
    Basis1 = np.reshape(Basis1,[sizeP, sizeP, tranNum, p1*p1])*np.expand_dims(Mask,3)
    Basis2 = np.reshape(Basis2,[sizeP, sizeP, tranNum, p1*p])*np.expand_dims(Mask,3)
    Basis3 = np.reshape(Basis3,[sizeP, sizeP, tranNum, p1*p])*np.expand_dims(Mask,3)
    Basis4 = np.reshape(Basis4,[sizeP, sizeP, tranNum, p*p])*np.expand_dims(Mask,3)
    Rank = 0
    # Weight = 1
    BasisR = np.concatenate([Basis1, Basis2, Basis3, Basis4], axis = 3)
    BasisR = np.reshape(BasisR, [sizeP*sizeP*tranNum, (p+p1)*(p+p1)])
