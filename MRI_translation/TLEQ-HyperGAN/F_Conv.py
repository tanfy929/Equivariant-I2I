...
class Fconv_PCA(nn.Module):
    ...
    def __init__(self,  sizeP, inNum, outNum, tranNum=4, inP = None, padding=None, ifIni=0, bias=True, Smooth = True, iniScale = 1.0, stride = 1):
        ...
        self.expand = expand
        self.weights = nn.Parameter(torch.Tensor(outNum, inNum, expand, 2*inP*inP), requires_grad=True)
...
class Fconv_PCA_out(nn.Module):
    ...
    def __init__(self,  sizeP, inNum, outNum, tranNum=4, inP = None, padding=None, ifIni=0, bias=True, Smooth = True,iniScale = 1.0, stride = 1):
        ...
        self.weights = nn.Parameter(torch.Tensor(outNum, inNum, 1, 2*inP*inP), requires_grad=True)
        # nn.init.kaiming_uniform_(self.weights, a=0,mode='fan_in', nonlinearity='leaky_relu')
...
class GetBasis(nn.Module):
    ...
    def forward(self):
        ...
        Basis = torch.cat((BasisC,BasisS),dim = 3)
        # print('Basis', Basis.size()) # ([7, 7, 4, 98])
        return Basis
